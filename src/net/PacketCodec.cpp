...
bool ByteWriter::write_string(std::string_view text, std::size_t maximum_length) {
    if (text.size() > maximum_length ||
        text.size() > std::numeric_limits<std::uint8_t>::max()) {
        return false;
    }
...
std::optional<std::vector<std::uint8_t>> encode_packet(
    PacketHeader header,
    ...
    const bool written =
        ...
        writer.write_u8(static_cast<std::uint8_t>(header.type)) &&
        writer.write_u8(header.flags) && writer.write_u32(header.sequence) &&
        writer.write_u32(header.acknowledgement) &&
        writer.write_u32(header.acknowledgement_bits) &&
        writer.write_u32(header.server_tick) &&
...
std::optional<Packet> decode_packet(std::span<const std::uint8_t> datagram) {
    if (datagram.size() < encoded_header_bytes ||
        datagram.size() > maximum_datagram_bytes) {
        return std::nullopt;
    }
    ...
    if (!reader.read_u32(packet.header.magic) ||
        !reader.read_u16(packet.header.version) || !reader.read_u8(type) ||
        !reader.read_u8(packet.header.flags) ||
        ...
        !reader.read_u16(packet.header.payload_bytes) ||
        !reader.read_u16(packet.header.reserved)) {
        return std::nullopt;
    }

    if (packet.header.magic != protocol_magic ||
        packet.header.version != protocol_version ||
        type > static_cast<std::uint8_t>(PacketType::Disconnect) ||
        (packet.header.flags &
         static_cast<std::uint8_t>(~(PacketFlagReliable | PacketFlagFragment))) !=
            0U ||
        packet.header.reserved != 0U ||
        packet.header.payload_bytes != reader.remaining()) {
        return std::nullopt;
    }
...
std::optional<std::vector<std::uint8_t>> encode_hello(const HelloMessage& message) {
    ByteWriter writer;
    if (!writer.write_u64(message.client_nonce) ||
...
std::optional<HelloMessage> decode_hello(std::span<const std::uint8_t> payload) {
    ...
    if (!reader.read_u64(message.client_nonce) ||
        !reader.read_u32(message.build_id) ||
        !reader.read_string(message.player_name, maximum_player_name_bytes) ||
        !reader.empty()) {
        return std::nullopt;
    }
    return message;
...
std::optional<WelcomeMessage> decode_welcome(std::span<const std::uint8_t> payload) {
    ...
    ByteReader reader(payload);
    if (!reader.read_u64(message.client_nonce) ||
        !reader.read_u64(message.session_token) ||
        !reader.read_u64(message.world_seed) ||
        !reader.read_u32(message.server_tick) ||
        !reader.read_i32(message.world_width) ||
        !reader.read_i32(message.world_height) ||
        !reader.read_u16(message.tick_rate) ||
        !reader.read_u8(message.client_id) ||
        !reader.read_u8(message.maximum_clients) || !reader.empty() ||
        message.world_width <= 0 || message.world_height <= 0 ||
        message.world_width > maximum_network_world_dimension ||
        ...
        message.client_id > message.maximum_clients ||
        message.maximum_clients > maximum_players) {
        return std::nullopt;
    }
    return message;
}
...
std::optional<InputMessage> decode_input(std::span<const std::uint8_t> payload) {
    ...
    ByteReader reader(payload);
    if (!reader.read_u64(message.session_token) ||
        !reader.read_u32(message.input_sequence) ||
        ...
        !reader.read_u8(material) || !reader.read_u8(message.radius) ||
        !reader.empty() || kind > static_cast<std::uint8_t>(InputKind::Paint) ||
...
std::optional<SnapshotMessage> decode_snapshot(
    std::span<const std::uint8_t> payload) {
    SnapshotMessage message{};
    ByteReader reader(payload);
    if (!reader.read_u32(message.server_tick) ||
        !reader.read_u64(message.state_hash) ||
        !reader.read_u32(message.organism_population) ||
        !reader.read_u16(message.agent_count) ||
